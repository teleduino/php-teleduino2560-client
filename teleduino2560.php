<?php
/*
  teleduino2560.php - Teleduino2560 PHP Class
  Version 0.1.2
...
class Teleduino2560
{
		...
		if(ord($this->_data[1]) == 86)
		{
			for($i = 0; $i < 70; $i++)
			{
				...
				else
				{
					...
					$data_packet['values'][$i] += 256 * ord($this->_data[56 + (2 * ($i - 54))]);
					$data_packet['values'][$i] += ord($this->_data[57 + (2 * ($i - 54))]);
				}
			}
		}
		return $data_packet;
	...
	public function getPresets()
	{
		...
		$eeprom = $this->getEeprom(0, 200);
		if(!isset($eeprom['values'][0]) ||
			strlen($eeprom['values'][0]) != 200 ||
			substr($eeprom['values'][0], 0, 1) != '#' ||
			substr($eeprom['values'][0], 1, 1) != chr(1) ||
		...
		$eeprom2 = $this->getEeprom(200, 196);
		if(!isset($eeprom2['values'][0]) ||
				strlen($eeprom2['values'][0]) != 196)
		{
			$eeprom2['result'] = 0;
			$eeprom2['time'] += $eeprom['time'];
			$eeprom2['values'] = array();
			return $eeprom2;
		}
		$eeprom['time'] += $eeprom2['time'];
		$eeprom['values'][0] .= $eeprom2['values'][0];
		if(substr($eeprom['values'][0], 2, 1) == '#' &&
			substr($eeprom['values'][0], 73, 1) == '#' &&
			substr($eeprom['values'][0], 144, 1) == '#')
		{
			for($i = 0; $i < 70; $i++)
			{
				$pin_mode = ord(substr($eeprom['values'][0], 3 + $i, 1));
				$pin_output = ord(substr($eeprom['values'][0], 74 + $i, 1));
				switch($pin_mode)
				{
					case 0:
					case 1:
		...
		if(substr($eeprom['values'][0], 144, 1) == '#' &&
			substr($eeprom['values'][0], 161, 1) == '#' &&
			substr($eeprom['values'][0], 290, 1) == '#')
		{
			for($i = 0; $i < 4; $i++)
			{
				$pin_offset = null;
				...
				if($clock_pin >= 0 && $clock_pin <= 69 &&
					$data_pin >= 0 && $data_pin <= 69 &&
					$latch_pin >= 0 && $latch_pin <= 69)
				{
		...
		if(substr($eeprom['values'][0], 290, 1) == '#' &&
			substr($eeprom['values'][0], 295, 1) == '#')
		{
			for($i = 0; $i < 4; $i++)
			{
				$serial = null;
				switch(ord(substr($eeprom['values'][0], 291 + $i, 1)))
				{
					case 0:
						$serial = 300;
						break;
				...
				$presets['serial'][$i] = $serial;
			}
		}
		if(substr($eeprom['values'][0], 295, 1) == '#' &&
			substr($eeprom['values'][0], 344, 1) == '#' &&
			substr($eeprom['values'][0], 393, 1) == '#')
		{
			for($i = 0; $i < 48; $i++)
			{
				$pin = ord(substr($eeprom['values'][0], 296 + $i, 1));
				$position = ord(substr($eeprom['values'][0], 345 + $i, 1));
				if($pin >= 0 && $pin <= 69)
				{
		...
		if(substr($eeprom['values'][0], 393, 1) == '#' &&
			substr($eeprom['values'][0], 395, 1) == '#')
		{
			for($i = 0; $i < 1; $i++)
			{
				$define = ord(substr($eeprom['values'][0], 394 + $i, 1));
	...
	public function resetEeprom($quarter)
	{
		if($quarter > 3)
		{
			return false;
	...
	public function setEeprom($offset, $bytes)
	{
		...
		if($offset > 4095 || !is_array($bytes) || !count($bytes) || count($bytes) > 253)
		{
			return false;
		}
	...
	public function setPresets($presets)
	{
		...
		for($i = 0; $i < 70; $i++)
		{
			...
			if(isset($pin_mode) && intval($pin_mode) >= 0 && intval($pin_mode) <= 2)
			{
				...
				{
					$bytes[74 + $i] = chr(intval($presets['pin_outputs'][$i]));
				}
			}
		}
		$bytes[144] = '#';
		...
		for($i = 0; $i < 4; $i++)
		{
			...
			if(isset($clock_pin) && intval($clock_pin) >= 0 && intval($clock_pin) <= 69 &&
				isset($data_pin) && intval($data_pin) >= 0 && intval($data_pin) <= 69 &&
				isset($latch_pin) && intval($latch_pin) >= 0 && intval($latch_pin) <= 69)
			{
				$bytes[$pin_offset + 0] = chr($clock_pin);
				$bytes[$pin_offset + 1] = chr($data_pin);
				$bytes[$pin_offset + 2] = chr($latch_pin);
				if(isset($enable_pin) && intval($enable_pin) >= 0 && intval($enable_pin) <= 69)
				{
		...
		for($i = 0; $i < 48; $i++)
		{
			...
			if(isset($pin) && intval($pin) >= 0 && intval($pin) <= 69)
			{
				$bytes[296 + $i] = chr($pin);
				if(isset($position) && intval($position) >= 0 && intval($position) <= 180)
				{
					$bytes[345 + $i] = chr($position);
				}
			}
		}
		$bytes[393] = '#';
		for($i = 0; $i < 1; $i++)
		{
			...
			{
				$bytes[394 + $i] = chr($define);
			}
		}
		$bytes[395] = '#';
		$eeprom = $this->setEeprom(0, substr(implode('', $bytes), 0, 200));
		if(isset($eeprom['result']) && $eeprom['result'])
		{
			$eeprom2 = $this->setEeprom(200, substr(implode('', $bytes), 200));
			if(!isset($eeprom2['result']) || !isset($eeprom2['time']))
			{
				$eeprom['result'] = 0;
			}
			else
			{
				$eeprom['result'] = $eeprom['result'] && $eeprom2['result'] ? 1 : 0;
				$eeprom['time'] += $eeprom2['time'];
			}
		}
		return $eeprom;
