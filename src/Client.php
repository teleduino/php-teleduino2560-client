...
namespace Teleduino\Teleduino2560Client;
...
class Client
{
    ...
    public function getAllInputs()
    {
        ...
        $data_packet['result'] = ord($this->data[0]);
        $data_packet['time'] = $end - $start;
        if(ord($this->data[1]) == 86) {
            for($i = 0; $i < 70; $i++) {
                if($i < 54) {
                    $data_packet['values'][$i] = ord($this->data[2 + $i]);
                } else {
                    $data_packet['values'][$i] = 0;
                    $data_packet['values'][$i] += 256 * ord($this->data[56 + (2 * ($i - 54))]);
                    $data_packet['values'][$i] += ord($this->data[57 + (2 * ($i - 54))]);
                }
            }
        }
